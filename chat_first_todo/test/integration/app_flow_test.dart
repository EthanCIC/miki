...
void main() {
  group('App Flow Integration Tests', () {
    testWidgets('Complete app flow test from SplashScreen to HomeScreen',
      ...
      expect(find.text('您好，我是'), findsOneWidget,
          reason: "在 HomeScreen 上找不到 '您好，我是'");
      expect(find.text(AppConstants.appName), findsWidgets,
          reason: "在 HomeScreen 上找不到 AppName");
      expect(find.text('您的智能個人助理，隨時為您提供幫助'), findsOneWidget,
          reason: "在 HomeScreen 上找不到助理描述");

      // 檢查主題切換按鈕（允許 light_mode 或 dark_mode 其中之一存在）
      final hasLightModeInitial =
          find.byIcon(Icons.light_mode).evaluate().isNotEmpty;
      final hasDarkModeInitial =
          find.byIcon(Icons.dark_mode).evaluate().isNotEmpty;
      expect(hasLightModeInitial || hasDarkModeInitial, true,
          reason: "HomeScreen 初始主題圖示未找到");
      ...
      // 確認主題已切換
      final hasLightModeAfterTap =
          find.byIcon(Icons.light_mode).evaluate().isNotEmpty;
      final hasDarkModeAfterTap =
          find.byIcon(Icons.dark_mode).evaluate().isNotEmpty;
      if (hasDarkModeInitial) {
      ...
      expect(find.text('任務管理'), findsOneWidget,
          reason: "HomeScreen 找不到 '任務管理' 卡片");
      expect(find.text('智能對話'), findsOneWidget,
          reason: "HomeScreen 找不到 '智能對話' 卡片");

      // 測試 FAB 按鈕
      expect(find.byType(FloatingActionButton), findsOneWidget,
          reason: "HomeScreen 找不到 FAB");
      expect(find.byIcon(Icons.add), findsOneWidget,
          reason: "HomeScreen 找不到 FAB 圖示");
    });
  });
}
